function [dtCell_spd_essen_data,dtCell_spd_essen_data_excel,dtCell_leak_count_data] = speed_measure(session_data,position_data,param)
...
%     leak_msg = param.leak_msg; 
    leak_searching_flag = param.leak_searching_flag;
...
%% system message indexing
    ...
    %            'RwdEvent1:default','RwdEvent1:hit'};
...
%       message is not useable, thus, the method we used past is required
    ...
    dtCell_leak_count_data = {'Trial #','Context','Leak count before black start','Leak count after black start'};
    dtCell_idx = 1;
        
    h = waitbar(0,'Speed measurement. Please wait...');
    for idxIter = 1:1:length(trial_start_idx)
       % caution!! trial number in session_data is not accurate, thus, do
       % not use trial number in session_data
       dtCell_spd_essen_data{dtCell_idx+1,1} = session_data{trial_start_idx(idxIter),msg_col(1)};   % trial number
       dtCell_spd_essen_data{dtCell_idx+1,2} = session_data{trial_start_idx(idxIter),msg_col(2)};   % context number
       
       dtCell_leak_count_data{dtCell_idx+1,1} = session_data{trial_start_idx(idxIter),msg_col(1)};  % trial number
       dtCell_leak_count_data{dtCell_idx+1,2} = session_data{trial_start_idx(idxIter),msg_col(2)};  % context number
       ...
       if leak_searching_flag
            param.leak_count_ref = 'position';
            dtMat_leak_before_black_start = leaking_info_gather(trial_start_idx,black_start_idx,session_data,position_data,param);    % leaking count
       end
       oneTrial_idx = [];
       ...
       if leak_searching_flag     % reward was given
            param.leak_count_ref = 'time';
            dtMat_leak_after_black_start = leaking_info_gather(black_start_idx,trial_end_idx,session_data,position_data,param);    % leaking count
       
            % dtCell_leak_count_data: {'Trial #','Context','Leak count before black start','Leak count after black start'}
            if ~isempty(dtMat_leak_before_black_start)
                dtCell_leak_count_data{dtCell_idx+1,3} = dtMat_leak_before_black_start;  % leaking count
            else
                dtCell_leak_count_data{dtCell_idx+1,3} = NaN;  
            end
            if ~isempty(dtMat_leak_after_black_start)
                dtCell_leak_count_data{dtCell_idx+1,4} = dtMat_leak_after_black_start;   % leaking count
            else
                dtCell_leak_count_data{dtCell_idx+1,4} = NaN;
            end
       ...
       anti_speed = anticipation_area_speed(oneTrial_position_data);
